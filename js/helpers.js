...
function getCartItemId(productId, name, size, color) {
  const baseId = productId || name.toLowerCase().replace(/\s+/g, '-');
  return [baseId, size, color].filter(Boolean).join('|');
}

function createProductCard(product) {
  const finalPrice = calculateFinalPrice(product);
  const wished     = favorites.includes(product.id);
  const needsOptions = product.homeDrop && ((product.sizes && product.sizes.length) || (product.colors && product.colors.length));
    ...
    <div class="product-card" data-category="${product.category}" data-product-id="${product.id}">
      ...
      <div class="product-info">
        ...
        <button class="${needsOptions ? 'view-product-btn' : 'add-to-cart'}"
          ...
          ${product.isInStock ? (needsOptions ? 'View Sizes' : 'Add to Cart') : 'Out of Stock'}
        </button>
      </div>
    </div>`;
}
  ...
  container.innerHTML = cart.map(item => `
    <div class="cart-item" data-id="${item.id}">
      ...
      <div class="cart-item-details">
        ...
        ${(item.size || item.color) ? `<span class="cart-item-meta">${[item.size ? 'Size ' + item.size : '', item.color || ''].filter(Boolean).join(' / ')}</span>` : ''}
...
function attachAddToCartListeners() {
  document.querySelectorAll('.favorite-btn').forEach(btn => {
    ...
    const id        = getCartItemId(productId, name);
    ...
    if (existing) {
      ...
      cart.push({ id: id, productId: Number(productId), name: name, brand: brand, price: Number(price), img: img, qty: 1 });
  ...
  if (moveBtn) {
    moveBtn.addEventListener('click', function () {
      ...
      favorites.forEach(function (id) {
        ...
        if (!product) return;
        const cartId   = getCartItemId(product.id, product.name);
        ...
        if (existing) {
          ...
          cart.push({
            ...
            name: product.name,
            brand: product.brand,
            price: Number(calculateFinalPrice(product)),
            img: product.image,
            qty: 1
          });
        }
